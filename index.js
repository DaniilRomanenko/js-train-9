...
function findElementGreaterThan(arr, num) {
  ...
  // Якщо ні, повертаємо рядок "Число, яке задовольняє умову, не знайдено"
...
function findLastEvenNumber(arr) {
  ...
  // За допомогою метода reverse обертаємо масив і використовуємо метод find для знаходження першого парного числа.
  const lastEvenNumber = arr.findLast(element => element %2 === 0);

  // Перевертаємо масив назад, щоб повернути його до вихідного стану.
  arr.reverse();
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const resultString = arr2.join(', '); // Використовуйте потрібний роздільник
...
console.log(getPositiveArrayValues([1, -2, 3, -4, 5])); // Виведе [1, 3, 5]
...
function removeAndDouble(arr, startIdx) {
  ...
  const arrDoubled = arr.map(element => element);
...
function wordLengths(arr) {
  ...
  const arrDoubled = arr.map(word => word.length);
...
function expandArrayByN(arr, n) {
  ...
  var newArray = arr.flatMap(element => Array(n).fill(element));
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((longest, current) => {
    ...
    if (current.length > longest.length) {
      // Якщо поточне слово довше, встановлюємо його як нове найдовше слово
      return current;
    } else {
